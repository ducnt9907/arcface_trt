...
ICudaEngine *createEngine(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt){
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../arcface_r100.wts");
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    for(int i=1; i<3; i++){
        std::string lname = "layer1." + std::to_string(i);
        x = IBasicBlock(network, weightMap, *x->getOutput(0), 64, 1, lname);
    }
    x = IBasicBlock(network, weightMap, *x->getOutput(0), 128, 2, "layer2.0", 128);
    for(int i=1; i<13; i++){
        std::string lname = "layer2." + std::to_string(i);
        x = IBasicBlock(network, weightMap, *x->getOutput(0), 128, 1, lname);
    }
    ...
    for(int i=1; i<3; i++){
        std::string lname = "layer4." + std::to_string(i);
        x = IBasicBlock(network, weightMap, *x->getOutput(0), 512, 1, lname);
    }
    ...
    std::cout << "Your platform support fastfp16: " << builder->platformHasFastFp16() << std::endl;
...
int main(int argc, char **argv){
    ...
    if (std::string(argv[1]) == "-s"){
        ...
        std::ofstream p("arcface_r100.engine", std::ios::binary);
        if (!p){
        ...
        if (std::string(argv[1]) == "-d"){
            std::ifstream file("arcface_r100.engine", std::ios::binary);
            if (file.good()){
